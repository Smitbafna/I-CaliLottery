...
dfx canister create dapp3_frontend
...
LEDGER_INIT_ARG="(variant { Init = record { 
    ...
    initial_values = vec { 
        record { \"${INITIAL_ACCOUNT}\"; record { e8s = 100_000_000_000_000 } } 
    }; 
    send_whitelist = vec {}; 
...
dfx canister install dapp3_frontend --mode=install --argument '(null)'
DAPP3_FRONTEND_ID=$(dfx canister id dapp3_frontend)
...
echo "Frontend Canister Contract ID: ${DAPP3_FRONTEND_ID}"
echo -e "\nAccount Information:"
